...
/** Find an OAuth thread for a recipient (user-initiated: we sent first, no inbound yet).
 * Returns { connectionId, messageId } if found, so we can fetch the full thread including our sent message. */
export async function findOAuthThreadByRecipient(
  organizationId: string,
  toEmail: string
): Promise<{ connectionId: string; messageId: string } | null> {
  const admin = createAdminClient();
  ...
  for (const conn of connections as EmailConnection[]) {
    try {
      ...
      if (conn.provider === 'gmail') {
        const list = await searchGmailMessagesTo(accessToken, normalizedTo, 5);
        const first = list.messages?.[0];
        if (first) return { connectionId: conn.id, messageId: first.id };
      } else if (conn.provider === 'outlook') {
        const list = await searchOutlookMessagesTo(accessToken, normalizedTo, 5);
        const first = list[0];
        if (first?.id) return { connectionId: conn.id, messageId: first.id };
      }
    } catch {
